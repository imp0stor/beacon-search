...
echo "   Indexing events from ${#TEST_RELAYS[@]} relays..."
...
FACET_RESPONSE=$(curl -s -X POST "$BACKEND_URL/api/search" \
    -H "Content-Type: application/json" \
    -d '{
        "query": "nostr",
        "mode": "hybrid",
...
VECTOR_RESPONSE=$(curl -s -X POST "$BACKEND_URL/api/search" \
    -H "Content-Type: application/json" \
    -d '{
...
WOT_SEARCH=$(curl -s -X POST "$BACKEND_URL/api/search" \
    -H "Content-Type: application/json" \
    -d "{
        \"query\": \"bitcoin\",
        \"mode\": \"hybrid\",
        \"user_pubkey\": \"$WOT_TEST_PUBKEY\",
        \"limit\": 5
    }")
